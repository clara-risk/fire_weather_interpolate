...
years = [] 
for x in range(1987,1988): #Insert years here 
   years.append(str(x))
overall_dates = []   
...
for var in variables: 
   ...
   for input_date in sorted(overall_dates):
      ...
      if var != 'pcp':
         ...
         #else:
            #MAE = gpr.shuffle_split_gpr(hourly_dictionary,temperature,shapefile,file_path_elev,elev_array,idx_list,0.3,10)
      ...
      if var == 'pcp':
         #MAE = gpr.shuffle_split_gpr(daily_dictionary,temperature,shapefile,file_path_elev,elev_array,idx_list,0.3,10)
   ...
   df.to_csv(file_out+'RF_ShuffleSplit_'+var+'.csv', header=None, sep=',', mode='a')
