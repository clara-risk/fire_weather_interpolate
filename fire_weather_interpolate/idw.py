...
def IDW(latlon_dict, Cvar_dict, input_date,
   ...
   ----------
    ...
    if show:
        ...
        plt.imshow(idw_grid, extent=(xProj_extent.min(
       ...
       y_origin_list = []
        ...
        for station_name_hold_back in station_name_list:
            ...
            Yi = np.linspace(np.min(yProj_extent),
                             np.max(yProj_extent), num_row)
            Xi = np.linspace(np.min(xProj_extent),
                             np.max(xProj_extent), num_col)
            ...
            # what if distance is 0 --> np.inf? have to account for the pixel
            # underneath
            weights = 1 / (distance_matrix**d)
            # Making sure to assign the value of the weather station above the
            # pixel directly to the pixel underneath
            weights[np.where(np.isinf(weights))] = 1 / (1.0E-50)
            weights /= weights.sum(axis=0)
