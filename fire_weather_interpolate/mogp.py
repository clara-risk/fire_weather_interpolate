...
def MOGP_interpolator(latlon_dict,daily_dict, Cvar_dict, Cvar_dict2, Cvar_dict3, Cvar_dict4, \
                      input_date, var_name, shapefile, show,\
...
def shuffle_split_mogp(loc_dict, Cvar_dict, Cvar_dict2, \
                        Cvar_dict3, Cvar_dict4, shapefile, \
                       file_path_elev, idx_list, expand_area, rep, show, res=10000):
   ...
   '''
    ...
    while count <= rep:  # Loop through each block/cluster, leaving whole cluster out
        ...
        error_dictionaryt[count] = sum(temp_absolute_error_dictionary.values(
        ))/len(temp_absolute_error_dictionary.values())  # average of all the withheld stations
        error_dictionaryr[count] = sum(rh_absolute_error_dictionary.values(
        ))/len(rh_absolute_error_dictionary.values())
        error_dictionaryw[count] = sum(wind_absolute_error_dictionary.values(
        ))/len(wind_absolute_error_dictionary.values())
        error_dictionaryp[count] = sum(pcp_absolute_error_dictionary.values(
        ))/len(pcp_absolute_error_dictionary.values()) 
        count += 1

    overall_errort = sum(error_dictionaryt.values()) / \
        rep  # average of all the runs
    overall_errorr = sum(error_dictionaryr.values()) / \
        rep
    overall_errorw = sum(error_dictionaryw.values()) / \
        rep
    overall_errorp = sum(error_dictionaryp.values()) / \
        rep  

    return overall_errort, overall_errorr,overall_errorw,overall_errorp
