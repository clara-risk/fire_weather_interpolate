     ...
     for method in interpolation_types[var_name]:
         if method not in ['IDW2','IDW3','IDW4','IDEW2','IDEW3','IDEW4','TPS','GPR','RF']:
             ...
             if method == 'IDW2':
                 ...
                 MAE = idw.cross_validate_IDW(loc_dictionary,cvar_dictionary, shapefile, 2, False,False)
                 MAE_dict[method] = MAE
                 
             if method == 'IDW3':
                 #MAE = idw.shuffle_split(loc_dictionary,cvar_dictionary,shapefile,3,rep,False,res=10000)
                 MAE = idw.cross_validate_IDW(loc_dictionary,cvar_dictionary, shapefile, 3, False,False)
                 MAE_dict[method] = MAE
                
             if method == 'IDW4':
                 #MAE = idw.shuffle_split(loc_dictionary,cvar_dictionary,shapefile,4,rep,False,res=10000)
                 MAE = idw.cross_validate_IDW(loc_dictionary,cvar_dictionary, shapefile, 4, False,False)
                 MAE_dict[method] = MAE
             ...
             if method == 'TPS':
                 ...
                 MAE= tps.cross_validate_tps(loc_dictionary,cvar_dictionary,shapefile,0,False) #no smoothing - just a test 
                 MAE_dict[method] = MAE

             if method == 'RF':
                 #MAE = rf.shuffle_split_rf(loc_dictionary,cvar_dictionary,shapefile,file_path_elev,elev_array,idx_list,10,res=10000)
                 MAE = rf.cross_validate_rf(loc_dictionary,cvar_dictionary,shapefile,file_path_elev,elev_array,idx_list,False)
                 MAE_dict[method] = MAE
             ...
             if method == 'GPR':
                 ...
                 MAE = gpr.cross_validate_gpr(loc_dictionary,cvar_dictionary,shapefile,file_path_elev,elev_array,idx_list,kernels[var_name]) 
                 MAE_dict[method] = MAE
...
def read_data(nd_array,res,shp,day):
     ...
     output_file = driver.Create('outputs/1976/'+str(day)+'.tif', num_col, num_row, 1, gdal.GDT_Float32) 
...
##
##        year = str(1976)
##        count = 0 #1 for DC, DMC, FFMC
##        
##        with open(save+year+'_FWI_auto_select.json', 'r') as fp:
##             isi_list = json.load(fp)
##             for dat in range(62,62+len(isi_list)+1): 
##                  read_data(isi_list[count],10000,shapefile,dat)
##                  count+=1
##
##        #resample(30)
